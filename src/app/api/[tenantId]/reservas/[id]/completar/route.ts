...
import { completeReservation } from "@/lib/db/reservations";
...
export async function PATCH(_request: Request, ctx: Ctx) {
  ...
  try {
    await completeReservation(gate.tenantUuid, id);
    return NextResponse.json({ message: "Reserva completada" });
  } catch (e) {
    const msg = e instanceof Error ? e.message : "";
    if (msg === "Reserva no encontrada") {
      return NextResponse.json({ error: "No encontrada" }, { status: 404 });
    }
    console.error("[PATCH reservas/[id]/completar]", e);
